...
Heap<HeapType>::Heap(HeapType* d, int32_t n) {
...
template <typename HeapType>
void Heap<HeapType>::insert(HeapType key) {
    int32_t
        c, p;
...
HeapType Heap<HeapType>::remove(int32_t pos) {
    ...
    if (pos < 0 || pos >= nElements)
        throw std::out_of_range("Illegal index " + std::to_string(pos));
