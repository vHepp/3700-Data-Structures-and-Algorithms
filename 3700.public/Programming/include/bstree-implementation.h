...
int32_t SortedLinearList<TreeType>::search(TreeType key) {
    ...
    throw domain_error("Key ["+to_string(key)+"] not found");
}
...
TreeType &SortedLinearList<TreeType>::operator[](int32_t pos) {
    ...
    if (root == nullptr || pos < 0 || pos >= root->count)
        throw out_of_range("Index " + std::to_string(pos) + " is out of range");
...
template <class TreeType>
TreeNode<TreeType> *SortedLinearList<TreeType>::prvInsert(TreeNode<TreeType> *r,
        TreeNode<TreeType> *newNode) {
...
template <class TreeType>
TreeNode<TreeType> *SortedLinearList<TreeType>::prvRemove(TreeNode<TreeType> *r,
        const TreeType &key) {
    TreeNode<TreeType>
        *tmpNode;
    TreeType
